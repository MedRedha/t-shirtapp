...
(function(){
    ...
    clientDialogController.$inject=['Client', 'DateUtils','$stateParams','$state']

    function clientDialogController(Client, DateUtils,$stateParams,$state){
        ...
        if($stateParams.id!=null) {
            vm.loadClient = function () {
                Client.get({id: $stateParams.id}, function (result) {
                    vm.client = result;
        ...
        vm.save = function () {
            ...
            if (vm.client.id != null) {
                Client.update(vm.client, function (data, headers) {
                    $state.go('client');
                ...
                Client.save(vm.client, function (data, headers) {
                    $state.go('client');
                }, function (httpResponse) {
                    console.log(httpResponse.headers('failure'));
                    vm.alerts.push({type: 'danger', msg: '' + httpResponse.headers('failure')});
